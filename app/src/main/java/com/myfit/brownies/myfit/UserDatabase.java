package com.myfit.brownies.myfit;

import android.content.ContentValues;
import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;

import java.util.ArrayList;
import java.util.List;

public class UserDatabase extends SQLiteOpenHelper {

    // sql.UserDatabase Version
    private static final int DATABASE_VERSION = 1;

    // sql.UserDatabase Name
    private static final String DATABASE_NAME = "UserManager.db";

    // model.User table name
    private static final String TABLE_USER = "user";

    // model.User Table Columns names
    private static final String COLUMN_USER_ID = "user_id";
    private static final String COLUMN_USER_NAME = "user_name";
    private static final String COLUMN_USER_EMAIL = "user_email";
    private static final String COLUMN_USER_PASSWORD = "user_password";
    private static final String COLUMN_USER_HEIGHT = "user_height";
    private static final String COLUMN_USER_WEIGHT = "user_weight";
    private static final String COLUMN_ACTIVITY = "user_activity";
    private static final String COLUMN_USER_AGE = "user_age";
    private static final String COLUMN_USER_SEX = "user_sex";
    private static final String COLUMN_GOAL = "user_goal";
    private static final String COLUMN_BMR = "user_bmr";



    // create table sql query
    private String CREATE_USER_TABLE = "CREATE TABLE " + TABLE_USER + " ("
            + COLUMN_USER_ID + " INTEGER PRIMARY KEY AUTOINCREMENT," + COLUMN_USER_NAME + " TEXT,"
            + COLUMN_USER_EMAIL + " TEXT," + COLUMN_USER_PASSWORD + " TEXT," + COLUMN_USER_HEIGHT + " INTEGER," + COLUMN_USER_WEIGHT + " INTEGER," + COLUMN_ACTIVITY + " TEXT," + COLUMN_USER_AGE + " INTEGER," + COLUMN_USER_SEX + " TEXT," + COLUMN_BMR + " INTEGER," + COLUMN_GOAL + " Text" + ")";

    // drop table sql query
    private String DROP_USER_TABLE = "DROP TABLE IF EXISTS " + TABLE_USER;

    /**
     * Constructor
     *
     * @param context
     */
    public UserDatabase(Context context) {
        super(context, DATABASE_NAME, null, DATABASE_VERSION);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(CREATE_USER_TABLE);
    }


    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {

        //Drop model.User Table if exist
        db.execSQL(DROP_USER_TABLE);

        // Create tables again
        onCreate(db);

    }

    /**
     * This method is to create user record
     *
     * @param user
     */
    public void addUser(User user) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(COLUMN_USER_NAME, user.getName());
        values.put(COLUMN_USER_EMAIL, user.getEmail());
        values.put(COLUMN_USER_PASSWORD, user.getPassword());
        values.put(COLUMN_USER_HEIGHT, user.getHeight());
        values.put(COLUMN_USER_WEIGHT, user.getWeight());
        values.put(COLUMN_ACTIVITY, user.getActivity());
        values.put(COLUMN_USER_AGE, user.getAge());
        values.put(COLUMN_USER_SEX, user.getSex());
        values.put(COLUMN_GOAL, user.getGoal());
        values.put(COLUMN_BMR, user.getBMR(user.getWeight(), user.getHeight(), user.getAge(), user.getSex(), user.getActivity(), user.getGoal()));


        // Inserting Row
        db.insert(TABLE_USER, null, values);
        db.close();
    }

    /**
     * This method is to fetch all user and return the list of user records
     *
     * @return list
     */
    public List<User> getAllUser() {
        // array of columns to fetch
        String[] columns = {
                COLUMN_USER_ID,
                COLUMN_USER_EMAIL,
                COLUMN_USER_NAME,
                COLUMN_USER_PASSWORD,
                COLUMN_USER_HEIGHT,
                COLUMN_USER_WEIGHT,
                COLUMN_USER_AGE,
                COLUMN_USER_SEX,
                COLUMN_BMR,
                COLUMN_GOAL

        };
        // sorting orders
        String sortOrder =
                COLUMN_USER_NAME + " ASC";
        List<User> userList = new ArrayList<User>();

        SQLiteDatabase db = this.getReadableDatabase();

        // query the user table
        /**
         * Here query function is used to fetch records from user table this function works like we use sql query.
         * SQL query equivalent to this query function is
         * SELECT user_id,user_name,user_email,user_password FROM user ORDER BY user_name;
         */
        Cursor cursor = db.query(TABLE_USER, //Table to query
                columns,    //columns to return
                null,        //columns for the WHERE clause
                null,        //The values for the WHERE clause
                null,       //group the rows
                null,       //filter by row groups
                sortOrder); //The sort order


        // Traversing through all rows and adding to list
        if (cursor.moveToFirst()) {
            do {
                User user = new User();
                user.setId(Integer.parseInt(cursor.getString(cursor.getColumnIndex(COLUMN_USER_ID))));
                user.setName(cursor.getString(cursor.getColumnIndex(COLUMN_USER_NAME)));
                user.setEmail(cursor.getString(cursor.getColumnIndex(COLUMN_USER_EMAIL)));
                user.setPassword(cursor.getString(cursor.getColumnIndex(COLUMN_USER_PASSWORD)));
                user.setHeight(cursor.getInt(cursor.getColumnIndex(COLUMN_USER_HEIGHT)));
                user.setWeight(cursor.getInt(cursor.getColumnIndex(COLUMN_USER_WEIGHT)));
                user.setActivity(cursor.getString(cursor.getColumnIndex(COLUMN_ACTIVITY)));
                user.setAge(cursor.getInt(cursor.getColumnIndex(COLUMN_USER_AGE)));
                user.setSex(cursor.getString(cursor.getColumnIndex(COLUMN_USER_SEX)));
                user.setGoal(cursor.getString(cursor.getColumnIndex(COLUMN_GOAL)));
                // Adding user record to list
                userList.add(user);
            } while (cursor.moveToNext());
        }
        cursor.close();
        db.close();

        // return user list
        return userList;
    }


    /**
     * This method to update user record
     *
     * @param user
     */
    public void updateUser(User user) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(COLUMN_USER_NAME, user.getName());
        values.put(COLUMN_USER_EMAIL, user.getEmail());
        values.put(COLUMN_USER_PASSWORD, user.getPassword());
        values.put(COLUMN_USER_HEIGHT, user.getHeight());
        values.put(COLUMN_USER_WEIGHT, user.getWeight());
        values.put(COLUMN_ACTIVITY, user.getActivity());
        values.put(COLUMN_USER_AGE, user.getAge());
        values.put(COLUMN_USER_SEX, user.getSex());
        values.put(COLUMN_BMR , user.getBMR(user.getWeight(), user.getHeight(), user.getAge(), user.getSex(), user.getActivity(), user.getGoal()));
        values.put(COLUMN_GOAL, user.getGoal());

        // updating row
        db.update(TABLE_USER, values, COLUMN_USER_ID + " = ?",
                new String[]{String.valueOf(user.getId())});
        db.close();
    }

    /**
     * This method is to delete user record
     *
     * @param user
     */
    public void deleteUser(User user) {
        SQLiteDatabase db = this.getWritableDatabase();
        // delete user record by id
        db.delete(TABLE_USER, COLUMN_USER_ID + " = ?",
                new String[]{String.valueOf(user.getId())});
        db.close();
    }

    /**
     * This method to check user exist or not
     *
     * @param email
     * @return true/false
     */
    public boolean checkUser(String email) {

        // array of columns to fetch
        String[] columns = {
                COLUMN_USER_ID
        };
        SQLiteDatabase db = this.getReadableDatabase();

        // selection criteria
        String selection = COLUMN_USER_EMAIL + " = ?";

        // selection argument
        String[] selectionArgs = {email};

        // query user table with condition
        /**
         * Here query function is used to fetch records from user table this function works like we use sql query.
         * SQL query equivalent to this query function is
         * SELECT user_id FROM user WHERE user_email = 'jack@androidtutorialshub.com';
         */
        Cursor cursor = db.query(TABLE_USER, //Table to query
                columns,                    //columns to return
                selection,                  //columns for the WHERE clause
                selectionArgs,              //The values for the WHERE clause
                null,                       //group the rows
                null,                      //filter by row groups
                null);                      //The sort order
        int cursorCount = cursor.getCount();
        cursor.close();
        db.close();

        if (cursorCount > 0) {
            return true;
        }

        return false;
    }
    public String getGoal (String username) {

        //String Answer = "";

        SQLiteDatabase db = this.getWritableDatabase();

        String [] column1 = {

                COLUMN_GOAL
        };

        String column2 = COLUMN_USER_EMAIL + "=?";

        String [] args = {username};
        // query the user table
        /**
         * Here query function is used to fetch records from user table this function works like we use sql query.
         * SQL query equivalent to this query function is
         * SELECT user_id,user_name,user_email,user_password FROM user ORDER BY user_name;
         */
        //Cursor cursor = db.rawQuery("SELECT COLUMN_BMR FROM TABLE_USER WHERE COLUMN_USER_EMAIL = ?", new String [] {username});

        Cursor cursor = db.query(TABLE_USER, //Table to query
                column1,                    //columns to return
                column2,                  //columns for the WHERE clause
                args,              //The values for the WHERE clause
                null,                       //group the rows
                null,                      //filter by row groups
                null);

        String goal = "";
        // Traversing through all rows and adding to list

        if (username != null && cursor.moveToFirst()) {
            goal = cursor.getString(cursor.getColumnIndex(COLUMN_GOAL));
        }

        cursor.close();
        db.close();

        // return user list
        return goal;
    }

    public String getActivity (String username) {

        //String Answer = "";

        SQLiteDatabase db = this.getWritableDatabase();

        String [] column1 = {

                COLUMN_ACTIVITY
        };

        String column2 = COLUMN_USER_EMAIL + "=?";

        String [] args = {username};
        // query the user table
        /**
         * Here query function is used to fetch records from user table this function works like we use sql query.
         * SQL query equivalent to this query function is
         * SELECT user_id,user_name,user_email,user_password FROM user ORDER BY user_name;
         */
        //Cursor cursor = db.rawQuery("SELECT COLUMN_BMR FROM TABLE_USER WHERE COLUMN_USER_EMAIL = ?", new String [] {username});

        Cursor cursor = db.query(TABLE_USER, //Table to query
                column1,                    //columns to return
                column2,                  //columns for the WHERE clause
                args,              //The values for the WHERE clause
                null,                       //group the rows
                null,                      //filter by row groups
                null);

        String Activity = "";
        // Traversing through all rows and adding to list

        if (username != null && cursor.moveToFirst()) {
            Activity = cursor.getString(cursor.getColumnIndex(COLUMN_ACTIVITY));
        }

        cursor.close();
        db.close();

        // return user list
        return Activity;
    }

    public int getBMR(String username) {

        //String Answer = "";

        String[] column1 = {

                COLUMN_BMR

        };


        SQLiteDatabase db = this.getWritableDatabase();

        String column2 = COLUMN_USER_EMAIL + "=?";

        //String [] args = {username};
        // query the user table
        /**
         * Here query function is used to fetch records from user table this function works like we use sql query.
         * SQL query equivalent to this query function is
         * SELECT user_id,user_name,user_email,user_password FROM user ORDER BY user_name;
         */
        //Cursor cursor = db.rawQuery("SELECT COLUMN_BMR FROM TABLE_USER WHERE COLUMN_USER_EMAIL = ?", new String [] {username});

        Cursor cursor = db.query(TABLE_USER, //Table to query
                column1,                   //columns to return
                column2,                  //columns for the WHERE clause
                new String[]{username},    //The values for the WHERE clause
                null,                       //group the rows
                null,                      //filter by row groups
                null);

        int BMR = 0;

        // Traversing through all rows and adding to list

        if ((cursor.moveToFirst()) && (username != null)) {

            BMR = cursor.getInt(cursor.getColumnIndex(COLUMN_BMR));

        }

        cursor.close();
        db.close();

        // return user list
        return BMR;
    }

    public int getWeight(String username) {

        //String Answer = "";

        String[] column1 = {

                COLUMN_USER_WEIGHT

        };


        SQLiteDatabase db = this.getWritableDatabase();

        String column2 = COLUMN_USER_EMAIL + "=?";

        //String [] args = {username};
        // query the user table
        /**
         * Here query function is used to fetch records from user table this function works like we use sql query.
         * SQL query equivalent to this query function is
         * SELECT user_id,user_name,user_email,user_password FROM user ORDER BY user_name;
         */
        //Cursor cursor = db.rawQuery("SELECT COLUMN_BMR FROM TABLE_USER WHERE COLUMN_USER_EMAIL = ?", new String [] {username});

        Cursor cursor = db.query(TABLE_USER, //Table to query
                column1,                   //columns to return
                column2,                  //columns for the WHERE clause
                new String[]{username},    //The values for the WHERE clause
                null,                       //group the rows
                null,                      //filter by row groups
                null);

        int Weight = 0;

        // Traversing through all rows and adding to list

        if ((cursor.moveToFirst()) && (username != null)) {

            Weight = cursor.getInt(cursor.getColumnIndex(COLUMN_USER_WEIGHT));

        }

        cursor.close();
        db.close();

        // return user list
        return Weight;
    }


    /**
     * This method to check user exist or not
     *
     * @param email
     * @param password
     * @return true/false
     */
    public boolean checkUser(String email, String password) {

        // array of columns to fetch
        String[] columns = {
                COLUMN_USER_ID
        };
        SQLiteDatabase db = this.getReadableDatabase();
        // selection criteria
        String selection = COLUMN_USER_EMAIL + " = ?" + " AND " + COLUMN_USER_PASSWORD + " = ?";

        // selection arguments
        String[] selectionArgs = {email, password};

        // query user table with conditions
        /**
         * Here query function is used to fetch records from user table this function works like we use sql query.
         * SQL query equivalent to this query function is
         * SELECT user_id FROM user WHERE user_email = 'jack@androidtutorialshub.com' AND user_password = 'qwerty';
         */
        Cursor cursor = db.query(TABLE_USER, //Table to query
                columns,                    //columns to return
                selection,                  //columns for the WHERE clause
                selectionArgs,              //The values for the WHERE clause
                null,                       //group the rows
                null,                       //filter by row groups
                null);                      //The sort order

        int cursorCount = cursor.getCount();

        cursor.close();
        db.close();
        if (cursorCount > 0) {
            return true;
        }

        return false;
    }

    @Override
    protected void finalize() throws Throwable {
        this.close();
        super.finalize();
    }


}